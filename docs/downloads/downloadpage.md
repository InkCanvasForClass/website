...
<div class="status-chips">
  <div class="status-chip smart-teach-chip">
    <span class="material-symbols-outlined">school</span>
    <span>智教镜像可用</span>
  </div>
  <button class="btn btn--tonal toggle-beta-btn" onclick="toggleBeta()">
    <span class="material-symbols-outlined">science</span>
    <span>显示测试版</span>
  </button>
</div>

<div id="release-loading" class="release-loading" style="display: none;">
  <div class="loader"></div>
  <p>正在加载...</p>
</div>

<div id="release-list" class="release-grid">
  <!-- 发布版本卡片将通过JavaScript动态加载 -->
</div>

<!-- 下载模态框 -->
<dialog class="modal" id="downloadModal">
  <div class="modal-content">
    ...
    <div class="modal-body">
      <p id="thank-you-text">感谢您下载 InkCanvasforClass-Community</p>
      <div class="countdown-container">
        <p>将在 <span id="countdown" class="countdown-number">5</span> 秒后自动下载。</p>
      </div>
      <div class="btn-group">
        <a id="manual-download" class="btn btn--filled" href="#">
          单击此处下载
        </a>
      ...
      <p id="manual-download-tip" class="manual-download-tip" style="display: none;">如果自动下载未开始，请点击上方链接。</p>
    </div>
...
function convertDownloadUrl(url, isBeta) {
  if (url.endsWith('.exe')) {
    return fastestMirror ? url.replace("https://github.com/", `${fastestMirror}/https://github.com/`) : url
  }
...
function renderMarkdown(markdown) {
  ...
  return markdown ? markdown.replace(/\n/g, '<br>') : '<p class="card-subtitle">没有提供更新日志。</p>'
}

function renderReleaseCard(release) {
  const isBeta = release._isBeta || false
  const versionChip = isBeta ? 
    '<div class="chip chip--small">测试版</div>' : 
    '<div class="chip chip--small" style="background-color: var(--md-sys-color-primary-container); color: var(--md-sys-color-on-primary-container);">正式版</div>'
  
  const assetsHtml = release.assets.length > 0 ? 
    release.assets.map(asset => 
      `<button class="btn btn--tonal download-btn" onclick="showDownloadModal('${asset.browser_download_url}', ${isBeta}, '${asset.name}', ${asset.size})">
        <span class="material-symbols-outlined">download</span>
        <span>${asset.name} (${(asset.size / 1024 / 1024).toFixed(2)} MB)</span>
      </button>`
    ).join('') : 
    '<p class="typescale-body-medium card-subtitle">无可用附件</p>'
  ...
  return `
    <article class="release-item card card--filled">
      ...
      <div class="markdown-body card-subtitle">${renderMarkdown(release.body)}</div>
...
function renderReleases() {
  ...
  const releases = showingBeta ? 
    [...releasesOfficial.map(r => ({ ...r, _isBeta: false })), ...releasesBeta.map(r => ({ ...r, _isBeta: true }))] :
    releasesOfficial
  
  const sortedReleases = releases.sort((a, b) => new Date(b.published_at) - new Date(a.published_at))
  
  releaseList.innerHTML = sortedReleases.map(renderReleaseCard).join('')
}

// 弹窗逻辑
function showDownloadModal(originalUrl, isBeta, fileName, fileSize) {
  ...
  const manualTip = document.getElementById('manual-download-tip')
  
  manualDownload.href = effectiveUrl
...
onMounted(async () => {
  ...
  if (!smartTeachAvailable) {
    ...
    console.log(`最快GitHub镜像源: ${fastestMirror || '无'}`)
  }
