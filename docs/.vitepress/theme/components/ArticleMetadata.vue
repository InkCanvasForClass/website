...
const hasLastUpdated = computed(() => {
  ...
  return t && t > 0 && new Date(t).getFullYear() > 1970
})

const showMetadata = computed(() => {
  if (frontmatter.value.metadata === false) return false
  if (frontmatter.value.layout === 'home') return false
  
  const path = page.value.filePath
  if (path === 'download.md' || path === 'en/download.md') return false
...
<template>
    <div v-if="showMetadata" class="word">
        <p>
            <span v-if="hasLastUpdated">
                ...
                {{ lang === 'en-US' ? 'Updated: ' : '更新: ' }}{{ date.toLocaleString() }}&nbsp;&nbsp;&nbsp;
            </span>
            ...
            {{ lang === 'en-US' ? 'Words: ' : '字数: ' }}{{ wordCount }} {{ lang === 'en-US' ? 'words' : '字' }}&nbsp;&nbsp;&nbsp;
            ...
            {{ lang === 'en-US' ? 'Reading time: ' : '时长: ' }}{{ readTime }} {{ lang === 'en-US' ? 'min' : '分钟' }}
        </p>
    </div>
</template>
